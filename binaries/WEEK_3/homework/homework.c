...
void win()
{
    char *fileContents = malloc_set(0x40); 
    FILE *file = NULL;
    setreuid(geteuid(), -1); 
    if ((file = fopen("/flag", "r")) == NULL)
    {
...
int setup()
{
    ...
    /**
     *  We are assigning the address of tellAJoke to performAct (meaning a function pointer located on teh heap) 
     *  mainAct is assigned by the heap. 
     **/
    
    mainAct->performAct = tellAJoke;
    currentAct = mainAct;
   
    /**  The free messages will be put in a bin. 
     *   They can potentially be re-used. 
     **/ 
    free(message1);
    free(message3);

    /* Allocate a buffer of any size. We can potentially use memory that is put in a bin */
    printf("How long do you want the show description to be?\n");
    scanf("%d", &descLen);
    showDescription = malloc_set(descLen+8);
  
    /* Write to the buffer we just allocated  */
